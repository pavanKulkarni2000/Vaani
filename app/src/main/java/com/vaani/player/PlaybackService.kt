        ...
        override fun onPlayerCommandRequest(
            ...
            when (playerCommand) {
                COMMAND_SEEK_TO_NEXT, COMMAND_SEEK_TO_PREVIOUS, COMMAND_SEEK_TO_MEDIA_ITEM, COMMAND_CHANGE_MEDIA_ITEMS, COMMAND_STOP -> {
                    val endMedia = PlayerData.currentPlayList[session.player.currentMediaItemIndex]
                    endMedia.playBackProgress = PlayerUtil.getMediaProgress(endMedia, session.player.currentPosition)
            ...
            when (reason) {
                MEDIA_ITEM_TRANSITION_REASON_AUTO, MEDIA_ITEM_TRANSITION_REASON_SEEK, MEDIA_ITEM_TRANSITION_REASON_PLAYLIST_CHANGED -> {
                    if (reason == MEDIA_ITEM_TRANSITION_REASON_AUTO) {
                        Files.saveProgress(PlayerData.currentPlayList[player.previousMediaItemIndex].apply {
                            playBackProgress = 0f
                        })
                    }
